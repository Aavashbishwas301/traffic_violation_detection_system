  ...
  try {
    ...
    if (response.data.status === 'Completed') {
      ...
      if (fine) {
        ...
        await Violation.findByIdAndUpdate(fine.violationId._id, { status: 'Paid' });
...
const initiateEsewa = async (req, res) => {
  ...
  try {
    ...
    const product_code = process.env.ESEWA_SCD || 'EPAYTEST';
    const secret = process.env.ESEWA_SECRET || '8g8M8t8P8m8thPIY';
