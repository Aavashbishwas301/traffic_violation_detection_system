...
const PaymentStatus = () => {
  ...
  const [message, setMessage] = useState(
    "SYNCING WITH CENTRAL BANKING GATEWAY...",
  );
  const { user } = useAuth();
    ...
    const verifyPayment = async () => {
      ...
      try {
        if (pidx) {
          ...
          setStatus("failure");
          setMessage("INVALID SETTLEMENT PARAMETERS DETECTED.");
        }
      } catch (error) {
        setStatus("failure");
        setMessage("CENTRAL SYNC FAILURE. RETRY SEQUENCE INITIATED.");
      }
    };
          ...
          <div className="space-y-6 relative z-10">
            {status === "processing" && (
              <div className="flex flex-col items-center space-y-8">
                ...
                <h2 className="text-4xl font-black uppercase italic tracking-tighter text-primary-950">
                  Processing.
                </h2>
              </div>
            )}
            ...
            {status === "success" && (
              <div className="flex flex-col items-center space-y-8 animate-slide-up">
                ...
                <h2 className="text-4xl font-black uppercase italic tracking-tighter text-green-600">
                  Settled.
                </h2>
              </div>
            )}
            ...
            {status === "failure" && (
              <div className="flex flex-col items-center space-y-8 animate-slide-up">
                ...
                <h2 className="text-4xl font-black uppercase italic tracking-tighter text-accent-crimson">
                  Rejected.
                </h2>
              </div>
            )}
